...
- Atlas Revival + Domain Rename v1 + Atlas Typed Links UX v1 + Atlas Chronicle-Centered Model v1 + Atlas Chronicles Timeline v1 + Chronicle Editor UX v1 is implemented: the former World section is available in the sidebar as `Атлас`; backend `chronicles` are timeline containers with a dedicated in-Atlas editor, existing backend `events` are timeline events, and `locations`/`factions` are presented in UI as `Места` and `Организации`, where Места collect maps and Организации collect character participants through soft typed links over `entity_links`.
...
`Кузница историй` — web-платформа для подготовки материалов настольных RPG. Основной фокус проекта — рабочее пространство ведущего: кампании, сценарии, карты, персонажи, предметы и экспорт материалов.
...
- Graph Node Links Upgrade v2: связи graph-узлов с картами, персонажами, предметами, ассетами, местами, организациями и хроникой через `entity_links`, включая быстрые переходы к связанным материалам.
...
- Atlas Revival + Domain Rename v1 + Atlas Typed Links UX v1 + Atlas Chronicle-Centered Model v1 + Atlas Chronicles Timeline v1 + Chronicle Editor UX v1 поверх World module v1: backend/API для `chronicles`, `locations`, `factions` и `events` сохраняет ownership scope, а frontend показывает раздел `Атлас` как дашборд `Хроники`, `Места`, `Организации`, отдельный экран timeline-редактора хроники, CRUD, теги для событий/справочников и хронико-центричные typed-поля поверх `entity_links`.
...
- Universal Entity Links v2: общий API/UI для направленных связей между сценариями, картами, персонажами, предметами, ассетами, местами, организациями и хроникой.
...
- Связи между сущностями: `entity_links` используется и для graph-node связей с картами/персонажами/предметами/ассетами/местами/организациями/событиями хроники, и как универсальный directed layer между основными материалами. Generic UI показывает блок "Связанные материалы", поддерживает relation-типы, label, удаление и quick open для активных редакторов; Атлас использует мягкие typed-поля, где событие хроники связывает сценарии/места/организации, Места связывают карты, а Организации связывают участников.
...
`chronicles`, `locations`, `factions` и `events` поддерживаются World module v1 на backend/API уровне: записи привязаны к владельцу, а campaign context в новом UX задается на уровне хроники. В frontend они представлены как раздел `Атлас`: `Хроники` являются отдельными временными линиями с `start_label`, `end_label`, `step_size`, визуальным zoom и отдельным in-Atlas editor; `events` являются событиями хроники и размещаются на шкале как точки или диапазоны через `position/end_position` и текстовые метки; `events.campaign_id` сохраняется только как legacy compatibility field и не редактируется в новом UI. `Места` являются пространственными контейнерами карт; `Организации` являются контейнерами персонажей-участников. Связи идут через мягкие typed UI-поля поверх `entity_links` и не являются эксклюзивными.
